...
Acquisition::Acquisition(const auto& image1, const auto& image2)
    : image1_(image1), image2_(image2) {
  pillars = image1.getSize();
  deep = image2.getSize();
}

void Acquisition::resizeimage(const auto& image) {
  std::vector<Pixel> p;

  for (unsigned int r = 0; r < height_; ++r) {
    double by = static_cast<double>(height_) / static_cast<double>(deep.y);
    ...
    for (unsigned int c = 0; c < width_; ++c) {
      double bx = static_cast<double>(width_) / static_cast<double>(deep.x);
...
auto Acquisition::pattern1(const auto& image) {
...
auto Acquisition::pattern(const auto& image) {
...
//  assert(pattern(image1).size() == pattern(image2).size()); //li mettiamo nel
//  main?
